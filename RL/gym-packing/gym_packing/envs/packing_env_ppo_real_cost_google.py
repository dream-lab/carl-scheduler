...
class VMPackingEnvPPOGoogleRealCost(gym.Env):
    '''
    Observation:
        Type: Tuple, Discrete
    ...
    def valid_action_mask(self):
        ...
        _ , idx = np.unique(pm_state, axis=0, return_index=True)
        action_mask2 = np.array([0] * len(pm_state))
        action_mask2[idx] = 1
        return action_mask1 & action_mask2
    
    def valid_action_mask_without_clustering(self):
        pm_state = self.state[:-1]
        demand = self.state[-1]
        action_mask = (pm_state[:,1:] - demand[1:]) > 0 - self.eps
        action_mask1 = (action_mask.sum(axis=1)==4).astype(int)
